...
Cada região inicia lastreada na moeda fiduciária soberana local (por exemplo,
BRL), com paridade nominal bruta 1:1. A região só passa ao regime independente
quando o operador principal autoriza no contrato gestor após avaliação de
reservas, passivos, capital de risco e perda de estresse. Nesse regime, o preço
deixa de ser presumido e toda operação expressa em fiat exige cotação auditável.
