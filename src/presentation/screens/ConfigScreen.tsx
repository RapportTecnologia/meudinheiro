  ...
  return (
    <View style={styles.container}>
      ...
      {state.baseToken ? <View style={styles.card}>
        ...
        <Text>
          Regime: {state.baseToken.monetaryMode === 'fiat_pegged'
            ? `lastreado em ${state.baseToken.referenceCurrency} (paridade nominal 1:1)`
            : `independente, com preço por cotação`}
        </Text>
        <Text>Gestor global: {state.baseToken.policyManager}</Text>
        ...
        <Text style={styles.warning}>
          O regime monetário não é uma preferência local. O app validará no
          backend se a região está lastreada ou independente.
        </Text>
        <Button title="Validar e salvar" onPress={async () => {
          try {
            await state.configureBaseToken(token);
            setToken('');
          }
